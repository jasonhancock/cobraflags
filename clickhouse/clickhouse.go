...
func NewConfig(flagSet *pflag.FlagSet, opts ...Option) *Config {
	...
	c.Add(
		...
		flags.New(
			&c.Host,
			o.flagName("clickhouse-db-host"),
			"Database hostname or IP address",
			flags.Env(o.envName("CLICKHOUSE_DB_HOST")),
		...
		flags.New(
			&c.User,
			o.flagName("clickhouse-db-user"),
			"Datatabase username",
			flags.Env(o.envName("CLICKHOUSE_DB_USER")),
			flags.Required(),
		),

		flags.New(
			&c.Password,
			o.flagName("clickhouse-db-pass"),
			"Database password",
			flags.Env(o.envName("CLICKHOUSE_DB_PASSWORD")),
		),

		flags.New(
			&c.Name,
			o.flagName("clickhouse-db-name"),
			"Database name",
			flags.Env(o.envName("CLICKHOUSE_DB_NAME")),
		),

		flags.New(
			&c.Port,
			o.flagName("clickhouse-db-port"),
			"Database port",
			flags.Env(o.envName("CLICKHOUSE_DB_PORT")),
			flags.Default(9000),
			flags.Required(),
		),
		...
		flags.New(
			&c.SSLMode,
			o.flagName("clickhouse-db-ssl-mode"),
			"Database SSL mode",
			flags.Env(o.envName("CLICKHOUSE_DB_SSL_MODE")),
		...
		flags.New(
			&c.SSLCert,
			o.flagName("clickhouse-db-tls-cert"),
			"TLS client certificate",
			flags.Env(o.envName("CLICKHOUSE_DB_TLS_CERT")),
		),

		flags.New(
			&c.SSLKey,
			o.flagName("clickhouse-db-tls-key"),
			"TLS client private key",
			flags.Env(o.envName("CLICKHOUSE_DB_TLS_KEY")),
		),

		flags.New(
			&c.SSLRootCert,
			o.flagName("clickhouse-db-tls-ca-cert"),
			"TLS CA Certificate",
			flags.Env(o.envName("CLICKHOUSE_DB_TLS_CA_CERT")),
		),
		*/
	)
...
func (cfg *Config) DSN() (string, error) {
	...
	if cfg.User != "" {
		data.Add("username", cfg.User)
	}
	if cfg.Password != "" {
		data.Add("password", cfg.Password)
	}
	...
	*/
	...
	return "clickhouse://" + strings.Join(hosts, ",") + dbName + "?" + data.Encode(), nil
}
